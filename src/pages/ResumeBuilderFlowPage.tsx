...
const getRecommendation = (analysis: AnalysisResult | null) => {
  ...
  if (value.includes('yes') || analysis.score >= 70) {
    return {
      label: analysis.score >= 70 ? 'Good opportunity' : 'Recommended to apply',
      description:
        'This role looks realistic. Review the tailored CV before applying.',
...
export const ResumeBuilderFlowPage: React.FC = () => {
  ...
  const filteredCVs = useMemo(() => {
    ...
    return cvVersions.filter((cv) =>
      `${cv.name} ${cv.target_role || ''}`.toLowerCase().includes(term),
    );
  }, [cvVersions, search]);
  ...
  const handleSaveManualText = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        cv_text_extracted_at: extractedAt,
      })
      .eq('id', selectedCv.id)
      .eq('user_id', user.id);
  ...
  const handleAnalyse = async () => {
    ...
    if (!selectedCv.cv_text?.trim()) {
      setPageError(
        'This CV has no extracted text. Paste the CV text manually before analysing.',
      );
...
const handleUseGeneratedInResumeBuilder = () => {
  if (!generatedCvText.trim()) {
    setPageError('No generated CV found. Run the analysis first.');
    return;
  }

  if (result?.id && result.id !== 'unsaved') {
    navigate(`/resume-builder/${result.id}`);
    return;
  }

  const importPayload = {
    source: 'resume_builder_flow',
    analysisId: result?.id ?? null,
    cvVersionId: result?.cv_version_id ?? selectedCv?.id ?? null,
    jobTitle: result?.job_title ?? selectedCv?.target_role ?? '',
    companyName: result?.company_name ?? '',
    roleCategory: String(
      result?.extended_data?.detected_role_category ??
        result?.extended_data?.role_category ??
        '',
    ),
    score: result?.score ?? result?.overall_job_fit_score ?? null,
    generatedCv: generatedCvText,
    matchedKeywords: result?.matched_keywords ?? [],
    missingKeywords: result?.missing_keywords ?? [],
    recommendations:
      result?.suggestions ??
      ((result?.extended_data?.cv_improvement_actions as
        | string[]
        | undefined) ??
        []),
  ...
  navigate('/resume-builder/editor', {
    state: {
      resumeBuilderImport: importPayload,
    },
  });
};
  ...
  const handleAttachToApplication = async () => {
    ...
    const score = Number(result.score || result.overall_job_fit_score || 0);
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-3 max-w-2xl">
              Choose an existing CV, paste a job description, analyse the
              match, then generate and attach a tailored CV.
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        ...
                        {cv.last_score !== null && (
                          <span
                            className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${getScoreBadgeClass(
                              cv.last_score,
                            )}`}
                          >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {selectedCv.cv_text?.trim()
                        ? `${selectedCv.cv_text.trim().slice(0, 160)}${
                            selectedCv.cv_text.trim().length > 160 ? '...' : ''
                          }`
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
            ...
            {!result ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                ...
                <p className="font-semibold text-slate-800">No analysis yet</p>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-[170px_1fr] gap-5 items-center">
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-center">
                    ...
                    <span
                      className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold mt-4 ${getScoreBadgeClass(
                        result.score,
                      )}`}
                    >
                  ...
                  <div
                    className={`rounded-2xl border p-4 ${recommendation.className}`}
                  >
            ...
            {!generatedCvText.trim() ? (
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 mt-4">
                  ...
                  <button
                    type="button"
                    onClick={handleUseGeneratedInResumeBuilder}
                    disabled={!generatedCvText.trim()}
                ...
                <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 mt-1">
                        Save this CV score and analysis to one of your tracked
                        applications.
                      </p>
                    </div>
                  </div>
                    ...
                    <>
                      ...
                      {loadingApplications ? (
                        ...
                        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-5 text-center">
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Create an application first, then attach the
                            tailored CV.
                          </p>
                      ...
                      {selectedApplication && (
                        <p className="text-xs text-slate-500 mt-2">
                          Selected: {selectedApplication.role_title} ·{' '}
...
const KeywordSummary = ({
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <p className="text-sm text-slate-400">None returned.</p>
      ) : (
