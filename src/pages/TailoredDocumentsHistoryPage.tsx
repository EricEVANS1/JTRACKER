...
type TailoredDocumentType = 'analysis_generated' | 'saved_builder';

interface TailoredDocument {
  id: string;
  document_type: TailoredDocumentType;
...
export const TailoredDocumentsHistoryPage: React.FC = () => {
  ...
  const fetchDocuments = async () => {
    ...
    const analysisDocuments: TailoredDocument[] = ((analysisData || []) as any[]).map(
      (item) => ({
        id: item.id,
        document_type: 'analysis_generated',
        cv_version_id: item.cv_version_id,
    ...
    const { data: savedBuilderData, error: savedBuilderError } = await supabase
      .from('cv_versions')
      .select(
        'id, name, target_role, cv_text, structured_cv, last_score, last_analyzed_at, created_at'
      )
      .eq('user_id', user.id)
      .eq('structured_cv->>source', 'resume_builder')
      .order('created_at', { ascending: false });

    if (savedBuilderError) {
      setError(savedBuilderError.message);
      setDocuments(analysisDocuments);
      setLoading(false);
      return;
    }

    const savedBuilderDocuments: TailoredDocument[] = ((savedBuilderData || []) as any[]).map(
      (item) => ({
        id: item.id,
        document_type: 'saved_builder',
        cv_version_id: item.id,
        job_title:
          item.structured_cv?.job_title ||
          item.target_role ||
          item.name ||
          'Saved Resume Builder CV',
        company_name: item.structured_cv?.company_name || null,
        generated_cv: item.cv_text || '',
        score: item.last_score,
        created_at: item.created_at,
      })
    );

    const combinedDocuments = [...analysisDocuments, ...savedBuilderDocuments].sort(
      (a, b) =>
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((item) => {
      ...
      const typeLabel =
        item.document_type === 'saved_builder'
          ? 'saved resume builder cv'
          : 'cv intelligence result';

      return `${cvName} ${company} ${role} ${targetRole} ${typeLabel}`
        .toLowerCase()
        .includes(value);
    });
  ...
  const handleEdit = (document: TailoredDocument) => {
    if (document.document_type === 'saved_builder' && document.cv_version_id) {
      window.location.href = `/resume-builder/saved/${document.cv_version_id}`;
      return;
    }
  ...
  const handleDelete = async (document: TailoredDocument) => {
    const confirmed = window.confirm(
      document.document_type === 'saved_builder'
        ? 'Remove this saved Resume Builder CV from history? The CV version will remain in CV Manager, but it will no longer appear here.'
    ...
    <div className="w-full max-w-6xl overflow-hidden">
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-3 max-w-2xl">
          View, download, edit, or remove tailored CVs generated from CV Intelligence
          and saved from Resume Builder.
        </p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by company, role, CV name, or document type"
        ...
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Generate a tailored CV from CV Intelligence or save one from Resume
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2 text-xs text-slate-500">
                      <span>{document.company_name || 'N/A'}</span>
