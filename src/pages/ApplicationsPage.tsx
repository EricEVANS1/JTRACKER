...
const hasReachedInterview = (app: Application) =>
Boolean(
...
const payload: Record<string, string | boolean | number | OutcomeReason | null> = {};
...
payload.reached_interview = true;
payload.final_response_pending = true;
payload.interview_count = 1;
}
...
payload.final_response_pending = false;
payload.outcome_reason = 'offer_received';
}
...
payload.final_response_pending = false;
payload.rejected_after_interview = false;
payload.outcome_reason = 'rejected_before_interview';
...
payload.final_response_pending = false;
payload.outcome_reason = 'withdrawn';
}
...
payload.final_response_pending = false;
payload.outcome_reason = 'ghosted';
}
...
if (newStatus === 'assessment') {
  if (!app.assessment_received_at) update.assessment_received_at = now;
...
if (newStatus === 'final_interview') {
  ...
  if (!app.final_interview_started_at) update.final_interview_started_at = now;
  if (!app.interview_started_at) update.interview_started_at = now;
  if (!app.response_received_at) update.response_received_at = now;
}
...
if (newStatus === 'offer') {
  ...
  if (hasReachedInterview(app)) {
    update.reached_interview = true;
    update.interview_count = Math.max(app.interview_count || 0, 1);
  }

  if (!app.offer_received_at) update.offer_received_at = now;
  if (!app.response_received_at) update.response_received_at = now;
}

if (newStatus === 'rejected') {
  const hadInterview = hasReachedInterview(app);
  ...
  if (hadInterview) {
    ...
    update.interview_count = Math.max(app.interview_count || 0, 1);
  } else if (app.assessment_received_at || app.status === 'assessment') {
    update.rejected_after_interview = false;
  ...
  if (!app.rejected_at) update.rejected_at = now;
  if (!app.response_received_at) update.response_received_at = now;
}
...
if (newStatus === 'withdrawn') {
  ...
  if (!app.withdrawn_at) update.withdrawn_at = now;
}
...
if (newStatus === 'ghosted') {
  ...
  if (!app.ghosted_at) update.ghosted_at = now;
  if (!app.response_received_at) update.response_received_at = now;
}
...
updatePayload: Record<string, string | boolean | number | OutcomeReason | null>,
...
setApplications((prev) => {
  const shouldLeaveActivePage =
    updatePayload.archived === true || updatePayload.status === 'archived';

  if (shouldLeaveActivePage) {
    return prev.filter((app) => app.id !== applicationId);
  }
...
const successMessage =
  newStatus === 'rejected'
    ? 'Application marked as rejected and moved to archive.'
    : newStatus === 'archived'
      ? 'Application moved to archive.'
      : `Status updated to ${formatStatus(newStatus)}.`;
...
await updateApplication(
  ...
  `Application moved from ${formatStatus(application.status)} to ${formatStatus(newStatus)}.`
);


};

const handleMarkInterviewReached = async (applicationId: string) => {
...
await updateApplication(
  applicationId,
  {
    ...
    interview_count: Math.max(app.interview_count || 0, 1),
    interview_started_at: app.interview_started_at || now,
    response_received_at: app.response_received_at || now,
    last_status_changed_at: now,
    status_updated_at: now,
  },
...
await updateApplication(
  applicationId,
  {
    status: app.status === 'rejected' ? app.status : 'interview',
    reached_interview: true,
    rejected_after_interview: false,
    final_response_pending: true,
    interview_count: Math.max(app.interview_count || 0, 1),
    interview_started_at: app.interview_started_at || now,
    response_received_at: app.response_received_at || now,
  ...
  'Marked as awaiting final interview response.',
  'Awaiting final interview response',
  'Application marked as awaiting a final response after interview.'
);


};

const handleMarkRejectedAfterInterview = async (applicationId: string) => {
const app = applications.find((item) => item.id === applicationId);
if (!app) return;
...
await updateApplication(
  applicationId,
  {
    ...
    archived_at: now,
    reached_interview: true,
    rejected_after_interview: true,
    final_response_pending: false,
    interview_count: Math.max(app.interview_count || 0, 1),
    interview_started_at: app.interview_started_at || now,
    response_received_at: app.response_received_at || now,
    rejected_at: app.rejected_at || now,
    last_status_changed_at: now,
    status_updated_at: now,
    outcome_reason: 'rejected_after_interview',
  },
  'Marked as declined after interview and moved to archive.',
  'Declined after interview',
  'Application was marked as rejected after reaching interview stage and moved to archive.'
);
...
try {
  ...
  setMessage(
    status === 'rejected'
      ? 'Application created as rejected and moved to archive.'
...
return ( <div className="w-full max-w-full overflow-hidden"> <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8"> <div> <h2 className="text-2xl sm:text-3xl font-bold mb-1">Applications</h2> <p className="text-slate-500 text-sm sm:text-base">
Track active applications, CV versions, lifecycle dates, interview outcomes, and job-search progress. </p> </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_220px] gap-3">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search active role, company, or source..."
  ...
  {filteredApplications.length === 0 ? (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-10 text-center">
      ...
      <h3 className="text-lg font-semibold mb-1">No active applications found</h3>
      <p className="text-slate-500 text-sm">
        Add an application or check the archive for rejected and closed records.
      </p>
    ...
    <div className="space-y-4">
      {filteredApplications.map((app, index) => (
        <ApplicationCard
          ...
          onMarkInterviewReached={handleMarkInterviewReached}
          onMarkAwaitingFinalResponse={handleMarkAwaitingFinalResponse}
          onMarkRejectedAfterInterview={handleMarkRejectedAfterInterview}
        />
      ))}
    </div>
  )}
...
onShare,
onDelete,
onMarkInterviewReached,
onMarkAwaitingFinalResponse,
onMarkRejectedAfterInterview,
}: {
app: Application;
updating: boolean;
...
onMarkInterviewReached: (applicationId: string) => void;
...
const journeyClass = app.rejected_after_interview
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => ( <label className="block"> <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">
{label} </span>
{children} </label>
);
